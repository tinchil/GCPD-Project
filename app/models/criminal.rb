class Criminal < ApplicationRecord
  ...
  def check_to_see_if_current_suspect
    suspect_list = self.suspects.select{|s| s.dropped_on.nil? && s.investigation.date_closed.nil?}
    unless suspect_list.empty?
      errors.add(:base, "This person is currently a suspect in an open investigation and cannot be removed from the system.")
      return
    end

    self.suspects.each do |s|
      s.destroy!
    end
  end
